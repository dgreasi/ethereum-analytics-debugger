  ...
  .then(res => {
    ...
    for (var i = 0; i < steps.length - 1; i++) {
      ...
      const endBlockNumber = i < steps.length -2 ?
        steps[i+1]-1 :
        steps[i+1];
      stepCalls.push(() => syncGetVarsStep(startBlockNumber, endBlockNumber, contract));
    }
...
const syncGetVarsStep = function(startBlockNumber, endBlockNumber, contract_arg) {
 return new Promise((resolve, reject) => {
    ...
    for (var i = startBlockNumber; i <= endBlockNumber; i++) {
      ...
      if (checkCl == -1) {
        ...
        getContractVarPromises.push(getStorageAtBlock(i, contract_arg, dbBlocks[checkBL].timestamp));
      }
...
const getAccountTransactionsGasSpentClearings = function(startBlockNumber, endBlockNumber, contract_arg, nickname) {
...
const getTransactions = function(startBlockNumber, endBlockNumber, contract_arg, nickname) {
...
const getSpentGasOfAccount = function(startBlockNumber, endBlockNumber, account, nickname) {
...
const getBalancePerBlockOfAccount = function(startBlockNumber, endBlockNumber, account, nickname) {
  return new Promise((resolve, reject) => {
...
const getNumberOfTranscationsOfAccountPerBlock = function(startBlockNumber, endBlockNumber, account) {
  if (account != "") {
...
const checkStartEndInput = function(startBlockNumber, endBlockNumber, endOfBlockEth) {
...
const getClearingsThroughTime = function(startBlockNumber, endBlockNumber, contract_arg, nickname) { 
  var onj = {hex: contract_arg, name: (nickname ? nickname : contract_arg)};
  addToHistory(onj);
  ...
  return new Promise((resolve, reject) => {
    ...
    syncStep(startBlockNumber, endBlockNumber, 2).then(rs => {
      ...
      syncContractVars(startBlockNumber, endBlockNumber, contract_arg).then(res => {
        res = [];
...
const getStorageAtBlock = function(block, contract_arg, timestamp) {
  return new Promise((resolve, reject) => {
    ...
    promiseGetStorageAll.push(getStorageAtBlockQuantityCheck(block, contract_arg));
          ...
          if (ts.input.toString().includes("0x256a9ea1")) {
            ...
            if (first_clear) {
              if (ts.input.includes("0x0d31d41a")) { // generation
                ...
                generation = marketAdd(generation, quant, (pric > 300 ? 300 : pric));
                // generation.push({quantity: quant, price: pric});
              } else { // consumption
                console.log("CONSUMPTION Q,P: " + quant +', '+ pric);
                consumption = marketAdd(consumption, quant, (pric > 300 ? 300 : pric));
...
const getTransactionsByAccount = function(startBlockNumber, endBlockNumber, myaccount) {
  return new Promise((resolve, reject) => {
    var transactionsR = [];
    var getBlockPromises = [];
    var blockNumberPromise = web3.eth.getBlockNumber();

...
function getAccountTransactionsGasSpentClearings(
  startBlockNumber,
  endBlockNumber
) {
...
function getNumberOfTranscationsOfAccountPerBlock(
  startBlockNumber,
  endBlockNumber,
  account
) {
...
function getStorageAtBlockPrice(block) {
  return web3.eth.getStorageAt(contract, 6, block).catch(() => {
    return -99;
  });
}

function getStorageAtBlockQuantity(block) {
  return web3.eth.getStorageAt(contract, 5, block).catch(() => {
    return -99;
  });
}

function getStorageAtBlockType(block) {
  return web3.eth.getStorageAt(contract, 7, block).catch(() => {
    return -99;
  });
}
